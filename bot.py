...
async def admin_set(update, context):
    ...
    try:
        args = context.args
        uid = int(args[0]); field = args[1]; key = " ".join(args[2:-1]); val = int(args[-1])
        p = get_player(uid)
        if not p:
...
async def admin_broadcast(update, context):
    if not is_admin(update.effective_user.id): return
    if not context.args:
        await update.message.reply_text("فرمت: /broadcast <پیام>"); return
    msg = " ".join(context.args)
    players = get_all_players(); sent = 0
    for p in players:
        try:
            await context.bot.send_message(p[0], f"📢 *پیام مدیریت:*\n\n{msg}", parse_mode="Markdown"); sent += 1
        except: pass
...
def main():
    ...
    print("✅ ربات افترون v5 شروع به کار کرد!")
    app.run_polling()
